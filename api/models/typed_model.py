...
class Primitive(BaseModel):
    ...
    def validate_properties(self, id: str, schema: schema_models.Schema):
        ...
        for model_property in self.properties:
            if model_property not in map(lambda p: p.dtype, schema_primitive_properties.properties):
        ...
        for schema_property in schema_primitive_properties.properties:
            if not schema_property.dtype in self.properties and schema_property.default is None:
                raise ValueError(f"{id} missing property '{schema_property.dtype}'")
            else:
                schema_property_base = schema.properties[schema_property.dtype]
    ...
    def validate_schema(self, id: str, model: Dict[str, "CompPrimitive"], schema: schema_models.Schema) -> Optional["SchemaPropositionError"]:
        ...
        if not self.dtype in schema.primitives and not self.dtype in schema.composites:
            raise ValueError(f"Data type '{self.dtype}' set for {id} not found in schema")
        else:
            if self.definition == Definition.primitive:
                if self.dtype not in schema.primitives:
                    raise ValueError(f"Primitive data type '{self.dtype}' set for {id} not found in schema")
                else:
                    self.validate_properties(id, schema)

            elif self.definition == Definition.composite:
                if self.dtype not in schema.composites:
                    raise ValueError(f"Composite data type '{self.dtype}' set for {id} not found in schema")
                else:
                    self.validate_properties(id, schema)
                
                    # Includes checking argument one by one 
                    schema_composite = schema.composites[self.dtype]
                    ...
                    for argument in self.arguments:
                        if not argument in model:
                            ...
                            if not argument_node.dtype in map(lambda x: x.dtype, schema_composite.relation.items):
                                expecting_types = ", ".join(map(lambda item: item.dtype, schema_composite.relation.items))
                                raise ValueError(f"Expected argument '{argument}' to be of type [{expecting_types}], but got <{argument_node.dtype}>")
                            else:
                                argument_dtypes_count.setdefault(argument_node.dtype, 0)
                                argument_dtypes_count[argument_node.dtype] += 1

                    schema_dtypes_count = {}
                    for relation_item in schema_composite.relation.items:
                        if type(relation_item.quantifier) == int:
                            schema_dtypes_count.setdefault(relation_item.dtype, 0)
                            schema_dtypes_count[relation_item.dtype] += relation_item.quantifier
                        else:
                            schema_dtypes_count[relation_item.dtype] = relation_item.quantifier
                    ...
                    for schema_dtype, schema_quantifier in schema_dtypes_count.items():
                        ...
                        if type(schema_quantifier) == int:
                            ...
                            if schema_quantifier == schema_models.SchemaQuantifier.one_or_more:
                                if count < 1:
                                    raise ValueError(f"Requires <atleast 1> propositions of type '{relation_item.dtype}' ({id})")
                            elif schema_quantifier == schema_models.SchemaQuantifier.zero_or_one:
                                if count > 1:
                                    raise ValueError(f"Requires <atmost 1> propositions of type '{relation_item.dtype}' ({id})")
...
class Composite(Primitive):
    ...
    def delete_argument(self, id: str) -> "Composite":
        return Composite(
            ...
            arguments=list(filter(lambda x: x != id, self.arguments))
        )
...
class Model(BaseModel):
    ...
    def typecheck(self):
        ...
        for composite in self.data.composites.values():
            for argument in composite.arguments:
    ...
    def to_pldag(self) -> PLDAG:
        ...
        for primitive_id, primitive in self.data.primitives.items():
            ...
            if type(primitive_schema.dtype) == schema_models.SchemaPrimitiveDtype:
                ...
                elif primitive_schema.dtype == schema_models.SchemaPrimitiveDtype.boolean:
                    model.set_primitive(primitive_id)
                else:
                    raise ValueError(f"Unsupported primitive type: {primitive_schema.dtype}")
            elif type(primitive_schema.dtype) == schema_models.SchemaRangeType:
                model.set_primitive(primitive_id, complex(primitive_schema.dtype.lower, primitive_schema.dtype.upper))
        ...
        for composite_id, composite in self.data.composites.items():
            ...
            if type(composite_schema.relation.operator) == schema_models.SchemaLogicOperator:
                if composite_schema.relation.operator == schema_models.SchemaLogicOperator.and_:
                    argument_id_map[composite_id] = model.set_and(map_arguments_helper(composite.arguments), alias=composite_id)
                elif composite_schema.relation.operator == schema_models.SchemaLogicOperator.or_:
                    argument_id_map[composite_id] = model.set_or(map_arguments_helper(composite.arguments), alias=composite_id)
                elif composite_schema.relation.operator == schema_models.SchemaLogicOperator.not_:
                    argument_id_map[composite_id] = model.set_not(composite.arguments, alias=composite_id)
                elif composite_schema.relation.operator == schema_models.SchemaLogicOperator.imply:
                    lhs, rhs = map_arguments_helper(composite.arguments)
                    ...
                    argument_id_map[composite_id] = model.set_nand(map_arguments_helper(composite.arguments), alias=composite_id)
                elif composite_schema.relation.operator == schema_models.SchemaLogicOperator.nor_:
                    argument_id_map[composite_id] = model.set_nor(map_arguments_helper(composite.arguments), alias=composite_id)
                elif composite_schema.relation.operator == schema_models.SchemaLogicOperator.xor_:
                    argument_id_map[composite_id] = model.set_xor(map_arguments_helper(composite.arguments), alias=composite_id)
                else:
                ...
                if composite_schema.relation.operator == schema_models.SchemaValuedOperator.equal:
                    argument_id_map[composite_id] = model.set_equal(map_arguments_helper(composite.arguments), composite_schema.relation.value, alias=composite_id)
                elif composite_schema.relation.operator == schema_models.SchemaValuedOperator.atleast:
                    argument_id_map[composite_id] = model.set_atleast(map_arguments_helper(composite.arguments), composite_schema.relation.value, alias=composite_id)
                elif composite_schema.relation.operator == schema_models.SchemaValuedOperator.atmost:
                    argument_id_map[composite_id] = model.set_atmost(map_arguments_helper(composite.arguments), composite_schema.relation.value, alias=composite_id)
                else:
                    raise ValueError(f"Unsupported composite operator: {composite_schema.relation.operator}")
            else:
                raise ValueError(f"Unsupported composite operator: {composite_schema.relation.operator}")
